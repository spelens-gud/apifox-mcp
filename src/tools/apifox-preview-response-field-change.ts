...
import { patchResponseField } from "../openapi/patch-schema-field.js";
...
const toolModule: RegisterableModule = {
  type: "tool",
  name: "apifox_preview_response_field_change",
  description: "Preview an Apifox response schema field change and store it for later apply",
    ...
    server.tool(
      ...
      async (args) => {
        ...
        const change = pendingChanges.create({
          ...
          summary: `${result.action} response ${args.status} field ${args.fieldPath} on ${args.method.toUpperCase()} ${args.path}`,
          diff,
          document: minimal,
        });
